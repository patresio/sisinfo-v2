...
def suppliers(request):
    ...
    form = SupplierForm()
    form_contact = inlineformset_factory(Supplier, Contact, form=ContactForm, extra=2)
    context = {
        "suppliers": suppliers,
        "form": form,
    ...
    if request.method == "POST":
        if form.is_valid() and form_contact.is_valid():
            ...
            messages.add_message(
                request,
                constants.SUCCESS,
                f"O fornecedor {supplier.trade} foi atualizado com sucesso",
            )
            return redirect(
                reverse("suppliers:fornecedor_update", kwargs={"slug": slug})
            )
        messages.add_message(
            request,
            constants.WARNING,
...
def contact_supplier_delete(request, id):
    ...
    return redirect(
        reverse("suppliers:fornecedor_update", kwargs={"slug": supplier.slug})
    )
