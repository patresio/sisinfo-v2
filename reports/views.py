...
def report_update(request, slug):
    ...
    if request.user in [report.professional, report.pro_accountable]:
        form = ReportUpdateForm(request.POST or None, instance=report, request=request)
        ...
        if request.method == "POST":
            if form.is_valid() and form_material.is_valid():
                ...
                messages.add_message(
                    request,
                    constants.SUCCESS,
                    f"O Laudo {report.number_report} foi atualizado com sucesso.",
                )
                return redirect(reverse("reports:report_update", kwargs={"slug": slug}))
            messages.add_message(
                request, constants.ERROR, "Não foi possivel atualizar o laudo."
            )
            return redirect(reverse("reports:report_update", kwargs={"slug": slug}))
        context = {
            "report": report,
...
def material_report_delete(request, id):
    ...
    messages.add_message(
        request,
        constants.SUCCESS,
        f"O Item {material_report.material.name} foi excluido do laudo {report.number_report} com sucesso.",
    )
